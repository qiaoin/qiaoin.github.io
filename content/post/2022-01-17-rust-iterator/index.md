...
迭代器作为 GoF 23 种设计模式之一，在各个编程语言中广泛使用。在本文中，我们一起来探索 Rust 对迭代器的支持。首先明确 Rust 中的迭代器类型，接下来讲解从集合获取迭代器的三种方式，然后基于 Rust 提供的能力实现一个我们自己的迭代器（以实现 `our_flatten()` 为例）。在此基础上，为 `Iterator` trait 引入 extension traits，让所有迭代器都可以直接使用 `our_flatten()`，方便扩展。
...
Rust 标准库实现的迭代器依托于 `Iterator` trait，它定义了一组抽象接口（Abstraction），让使用者无需关心集合的底层实现细节，直接调用 `next()` 将集合作为迭代器（Iterator）进行访问，每次访问一个元素。
...
- 外部迭代器（external iterators），使用 `struct` 保存当前迭代的状态信息，由调用方来控制迭代行为（调用 `next()` 从迭代器中获取元素）。例如 `for-in-loops`，这个语法糖在后面会详细讲解；
- 内部迭代器（internal iterators），传递一个闭包（closures）给迭代器，迭代器在每个元素上调用这个闭包操作，无需保存当前迭代的状态信息，完全由迭代器来控制迭代行为。例如 `Iterator::for_each(self, f: F)`。

Rust 同时提供了内部迭代器和外部迭代器的语义，本文中我们主要聚焦在外部迭代器。对于内部迭代器，会与 `composable` 一起，搭配闭包（closures）和适配器（adapters）在下一篇文章中进行介绍。

### Iterator trait

所有迭代器（Iterator）都实现了 `Iterator` trait（查看目前标准库中实现了 `Iterator` trait 的 [Implementors](https://doc.rust-lang.org/std/iter/trait.Iterator.html#implementors)），其定义如下：
...
- 方法（methods），也称关联函数（associated functions）。实现 `Iterator` trait 必须实现 `next()`，定义了从迭代器中取下一个值的方法。当一个迭代器的 `next()` 方法返回 `None` 时，表明迭代器中已经没有值了。实现 `next()` 后，`Iterator` trait 中的其他方法就都有了缺省实现（其他方法的缺省实现使用了 `next()`，性能可能比较差；我们可以为其他方法提供自定义实现）。

## Three forms of iteration

从一个集合得到迭代器（create iterators from a collection），有三种方法：

- `iter()`, which iterates over `&T`.
- `iter_mut()`, which iterates over `&mut T`.
- `into_iter()`, which iterates over `T`.

注意，这里只是一个笼统的描述，实际情况需要根据标准库中具体类型来确认。例如，[HashSet](https://doc.rust-lang.org/std/collections/struct.HashSet.html) or [HashMap](https://doc.rust-lang.org/std/collections/struct.HashMap.html) 就不提供 `iter_mut()` 方法；`&str` 根据返回的迭代器类型，提供 [chars()](https://doc.rust-lang.org/std/primitive.str.html#method.chars) 和 [bytes()](https://doc.rust-lang.org/std/primitive.str.html#method.bytes) 方法。

以 `Vec<T>` 作为例子，写一段代码进行理解，[代码 1，three-forms-of-iteration](https://play.rust-lang.org/?version=stable&mode=debug&edition=2021&gist=83404e09bc12f1d203e43c20ca6bcfb3)：
...
`v` 的类型为 `Vec<T>`，为什么 `v.iter()` / `v.iter_mut()` 可以直接调用切片类型的方法呢？是由于 [Deref](https://doc.rust-lang.org/std/ops/trait.Deref.html) / [DerefMut](https://doc.rust-lang.org/std/ops/trait.DerefMut.html) trait —— 为一个类型（Type）实现 `Deref` / `DerefMut` trait，就可以像使用引用一样进行解引用（dereferencing）。
...
默认情况下（without  `Deref` / `DerefMut` trait），编译器只能对 `&` 执行解引用。有了  `Deref` / `DerefMut` trait ，`deref()` / `deref_mut()` 返回引用类型，此时编译器就知道了如何在一个类型（Type）上执行解引用（调用 `deref()` / `deref_mut()`  得到引用，再执行解引用得到对应的 `Self::Target` / `mut Self::Target` 类型）。
...
因此，`Vec<T>` 能够执行解引用，对应 `v.iter()` / `v.iter_mut()` 详细解释为：
...
`std::slice::Iter<'a, T>` 和 `std::slice::IterMut<'a, T>` 实现 `Iterator` trait，都是使用宏进行定义的（[impl-Iterator-for-Iter](https://doc.rust-lang.org/src/core/slice/iter.rs.html#134-144)、[impl-Iterator-for-IterMut](https://doc.rust-lang.org/src/core/slice/iter.rs.html#316)），关于宏，本系列会有单独一篇文章进行讲解。

 对于 `Vec<T>`、slice 和 `Iterator` trait之间的转换，可以借助下图进行理解。更深入的对 slice 的介绍，可以查看 [陈天 · Rust 编程第一课 - 第 16 讲](https://time.geekbang.org/column/article/422975)。
...
因此，查找方法名时就需要获取得到所有的候选类型（a list of candidate receiver types） —— 这一步是通过对 `x` 执行多次解引用来完成的。
...
fn main() {
    ...
    hello(&m);  // 场景 1：作为函数参数，&m type is &String -- auto dereferenece --> &str
...
除了使用 `*` 一元运算符进行**显式**解引用，在更多的场景下，编译器（Rust compiler）会**自动**进行隐式解引用（上述编译器执行解引用的情况 2 和 3。在 immutable 的上下文中，使用 `Deref`；在 mutable 的上下文中，使用 `DerefMut`），称为 `Deref coercion`。

如果没有 `Deref coercion`，就需要按照 `hello(&str)` 的签名去严格匹配（支持 `Deref coercion`，编译器会在编译期（compile time）自动进行转换），写出来的代码不易阅读；但同时也应该认识到，过分的依赖 `Deref` 会使代码不易维护，因此 [Deref](https://doc.rust-lang.org/std/ops/trait.Deref.html) / [DerefMut](https://doc.rust-lang.org/std/ops/trait.DerefMut.html) trait 的文档中都提到：
...
3、`v.into_iter()`，`std::vec::Vec` 实现 `IntoIterator` trait，能够将 `std::vec::Vec` 转换为迭代器 `std::vec::IntoIter`（实现了 `Iterator` trait）。
...
这里有一个疑问：为什么 Rust 不直接为 `Vec<T>` 实现 `Iterator` trait 呢，而是另外定义了一个 `IntoIter<T> struct` 来实现 `Iterator` trait 呢？

理由有以下两点：

1、回顾一下 `Iterator` trait `next(&mut self)` 的签名，为可变引用，基于 Rust 借用的规则，活跃的可变引用只能有一个。因此，如果为 `Vec<T>` 实现 `Iterator` trait，就不支持同时经由 `Vec<T>` 创建多个迭代器；并且由于可以在迭代期间修改迭代器，很容易出现错误，不符合 Rust 安全的宣言；

2、外部迭代器需要保存当前迭代的状态，此状态需要在每次迭代时更新。因此，如果为 `Vec<T>` 实现 `Iterator` trait，就需要在 `Vec<T>` 保存迭代状态，这样经由 `Vec<T>` 不可变引用上创建的迭代器，修改不了其保存的迭代状态，也就实现不了迭代器。

因此需要另外定义了一个 `IntoIter<T> struct` 来实现 `Iterator` trait。
...
Rust 语法糖，`in` 关键字后的表达式需要实现 `std::iter::IntoIterator` trait，`for` 循环遍历`.into_iter()` 返回的迭代器，当迭代器返回 `Some(val)` 时，按照 `Pattern` 匹配，然后执行 `for-block` 中的语句。`for` 循环多次执行，直至迭代器返回 `None` 或显式 `break`。
...
fn main() {
    ...
    println!("v1 = {:?}", v1);  // v1.iter() borrows immutable, so v1 can still access
    ...
    println!("v2 = {:?}", v2);  // v2.iter() borrows mutable, so v2 can still access
    ...
    println!("v4 = {:?}", v4);  // &v4 borrows immutable, so v4 can still access
    ...
    println!("v5 = {:?}", v5);  // &mut v5 borrows immutable, so v5 can still access
}
```

1、`v1` 和 `v2` 表示切片的迭代器（`std::slice::Iter struct` 和 `std::slice::IterMut struct`），都实现了 `Iterator` trait。而所有迭代器（实现了 `Iterator` trait 的类型），均实现了 `IntoIterator` trait（返回该迭代器本身），因此可以使用 `for-in-loops` 进行迭代访问：
...
3、`&v4` 和 `&mut v5` 对应 `Vec` 中对另外两个类型实现 `IntoIterator` trait，在 `into_iter(self)` 的实现中直接调用 `iter()` 和 `iter_mut()`；与 `v1` 和 `v2` 的处理逻辑等价
...
那自然会产生一个疑问：既然为 `&C` / `&mut C` 实现 `IntoIterator` trait，与 `C` 上的  `iter(&self)` / `iter_mut(&mut self)` 等价，为什么 Rust 要同时提供这两种方式呢？只提供一种方式是否可以？

基于以下两个方面考虑：

1、实现 `IntoIterator` trait 是使用 `for-in-loops` 的要求；当不在 `for-in-loops` 中使用时，使用 `v.iter()` 要比 `(&v).into_iter()` 更清晰，代码更易读；

2、`IntoIterator` trait 可以作为 trait bound，在范型编程时指定参数应该满足的要求，`T: IntoIterator` 表示参数类型 `T` 需要能够作为迭代器进行访问；或者，`T: IntoIterator<Item = U>` 表示被迭代的类型需要是类型 `U`；而 `iter(&self)` / `iter_mut(&mut self)` 表达不了 trait bounds 约束。例如下面这个例子（[代码 5，dump-function](https://play.rust-lang.org/?version=stable&mode=debug&edition=2021&gist=a413366a115c86dec36bb38021f3aeb4)），`dump` 函数将传入的可迭代变量依次打印出来，其 `Item` 需要实现 `Display` trait；`Vec<T>` 未实现 `Display` trait，因此 `dump(v3)`  会编译报错。
...
至此，第两部分结束，我们使用 `Vec<T>` 作为例子，分析了 `iter()` / `iter_mut()` / `into_iter()` 三种方式的原理：
...
基于前两部分的学习，我们来动手练习一下，实现一个自己的迭代器 —— `our_flatten()` —— 支持从前往后和从后往前同时遍历。

## Implementing our_flatten()

首先，我们来看一下 `Iterator` trait 中 [flatten()](https://doc.rust-lang.org/std/iter/trait.Iterator.html#method.flatten) 方法的功能，将一个嵌套的迭代器往下平铺**一层**（one level down），[代码 6，flatten-one-level-down](https://play.rust-lang.org/?version=stable&mode=debug&edition=2021&gist=2fb3fe9722d21cfa00868701bbfcb2fa)：
...
`data1` / `data2`  的每一个元素类型为 `Vec<T>`（`outer::Item`），实现了 `IntoIterator` trait，都可以调用 `.flatten()` 将嵌套的迭代器向下平铺一层 —— `data1.into_iter().flatten()` 得到的迭代器，其遍历的元素类型为 `T`，而 `data2.into_iter().flatten()` 得到的迭代器，其遍历的元素类型为 `Vec<T>`。`flatten()` 只往下平铺一层，就是这个含义，如果需要再往下一层，可以多次进行调用 `data2.into_inter().flatten().flatten()`。

弄清楚了标准库 `std::iter::Iterator::flatten()` 的功能，接下来我们看看如何实现该功能。

### version #1: set up

`flatten(iter)` 的入参需遵循以下要求：

- `iter` 为迭代器（实现 `Iterator` trait）或者可以被迭代（实现 `IntoIterator` trait）；
- `iter::Item` 可以被迭代（实现 `IntoIterator` trait）；

![flatten-outer-item-bounds](images/flatten-outer-item-bounds.png)

第一层 `outer` 调用 `next()` 获取 `Option<Item>`，当存在待处理的值时 —— `Some(inner)` —— 调用 `.into_iter().next()`。这样我们就实现了 `flatten()` 的第一个版本，[代码 7，version #1: set-up](https://play.rust-lang.org/?version=stable&mode=debug&edition=2021&gist=41717cc3d5ab80bb07b0ba7700ef5b3f)。
...
将 `and_then()` 使用 `?` 展开，问题会更清晰：每次调用 `next`，`outer` 都往前走了一步，即使 `inner_item` 存在多个元素待处理。
...
我们需要保存当前正在迭代的 `inner_iter`，从 `inner_item.into_iter()` 可知其类型为 `<O::Item as IntoIterator>::IntoIter`；同时在 `inner_iter` 未开始迭代和迭代完成时，需要切换迭代 `outer`，因此使用 `Option` 修饰 `inner_iter`：
...
3、处理内层迭代器 `inner`，在仍然有待处理的元素时，返回该元素；在返回 `None` 时，将 `inner` 设置为 `None`。外层迭代器的第一个元素迭代结束;

4、`outer.next()` 返回外层迭代器的后续元素，继续处理。

得到 `flatten()` 的第二个版本，[代码 8，version #2: save-inner-iter](https://play.rust-lang.org/?version=stable&mode=debug&edition=2021&gist=7e2f1df53f70a2af30d02147f8fb3d31)，主要逻辑在 `next()` 的 `loop` 循环中，可以按照上述逻辑比对代码认真理解。
...
`Iterator` trait，我们已经很了解了，实现 `Iterator` trait 后，可以使用 `.next()` 从前往后进行迭代；那什么是 `DoubleEndedIterator` trait 呢？望文生义，既然是 `DoubleEnded`，实现了  `DoubleEndedIterator` trait 后是不是就可以从后往前迭代了？
...
需要认真考虑的是在 `flatten_iter` 上同时调用 `next()` 和 `next_back()`，前面已经提到 `next()` 和 `next_back()` 处理的是相同 Range，增加以下测试用例，测试报错：
...
分析一下原因，在外层迭代器上调用 `next()` 后，`inner` 保存的是第一个元素的内层迭代器，`next()` 和 `next_back()` 都是基于 `inner` 进行处理，对于 `next()` 返回符合预期，但 `next_back()` 返回的是 `inner` 迭代器最后一个元素，不符合预期（预期是返回外层迭代器最后一个元素（其作为内部迭代器）的最后一个元素）。
...
由于 `next()` 和 `next_back()` 处理的是相同的 Range，在迭代处理的最后，会出现两种类似的情况：
...
基于以上分析，我们得到 `flatten()` 的第四个版本，[代码 11，version #4: save-front_iter-and-back_iter](https://play.rust-lang.org/?version=stable&mode=debug&edition=2021&gist=f16e1494e0f61fa913876969439bd54d)。`next()` 和 `next_back()` 的主体逻辑与 version #2 和 version #3 一致，以 `next_back()` 的实现为例：
    ...
    fn next(&mut self) -> Option<Self::Item> {
        loop {
            ...
            if let Some(next_inner) = self.outer.next() {
                ...
                // 处理图示情况 2
                // back_iter 被调用过一次，consume 了一个 outer Item
                // 但 back_iter 还有内容可以遍历, 此时调用 front_iter.next() 时需要返回数据
    ...
    fn next_back(&mut self) -> Option<Self::Item> {
        loop {
            ...
            if let Some(next_back_inner) = self.outer.next_back() {
                ...
                // 处理图示情况 1
                // front_iter 被调用过一次，consume 了一个 outer Item
                // 但 front_iter 还有内容可以遍历, 此时调用 back_iter.next_back() 时需要返回数据
...
`next(&mut self)` 和 `next_back(&mut self)` 都是可变引用（mutable references），因此不能够并发进行调用（活跃的可变引用只能存在一个，独占）。

至此，我们实现了自己的 `flatten(iter)` 🎉，入参迭代器作为函数参数传入，[代码 11，version #4: save-front_iter-and-back_iter](https://play.rust-lang.org/?version=stable&mode=debug&edition=2021&gist=f16e1494e0f61fa913876969439bd54d)。
...
为了让 API 更好用（Rust 中称为语言的人体工程学，[Rust's language ergonomics initiative](https://blog.rust-lang.org/2017/03/02/lang-ergonomics.html)），直接作为迭代器的方法调用 `iter.flatten()`，需要引入 [extension traits](https://github.com/rust-lang/rfcs/blob/master/text/0445-extension-trait-conventions.md)。

对于 extension traits，目前找到的介绍来自 Rust for Rustaceans Chapter 13.2 Pattern in the Wild：
...
这段话中提到了两个例子，先来看一下这两个例子是如何使用 extension traits 的。
...
`Iterator` trait 是 `Itertools` trait 的 [supertrait](https://doc.rust-lang.org/book/ch19-03-advanced-traits.html#using-supertraits-to-require-one-traits-functionality-within-another-trait)（本文前面提到的 `trait DerefMut: Deref` 和 `trait DoubleEndedIterator: Iterator` 都是如此），可以使用面向对象中的“**继承（inheriting）**”进行理解，`trait B: A`，是说任何类型 `T`，如果实现了 trait B（`impl B for T`），它也必须实现 trait A（`impl A for T`），换句话说，**trait B 在定义时可以使用 trait A 中的关联类型和方法**。

定义好 `Itertools` trait 后，提供方法的缺省实现，再为外部类型 `T`（使用 `where T: Iterator` 子句为 `T` 添加 trait bounds）实现 `Itertools` trait —— 即对于所有的满足 `Iterator` trait bounds 的类型 `T`，都实现了 `Itertools` trait —— 因此所有的迭代器（实现了 `Iterator` trait）均直接使用 `Itertools` trait 中的所有方法，相当于对 `Iterator` trait 进行了扩展（extension）。
...
2、为外部类型实现步骤 1 中定义的 trait（`impl<T: ?Sized> XxxxExt for T where T: ExternalType { ... }`）。

### blanket implementations

为指定了 trait bounds 的外部类型 `T` 实现自定义的 `trait XxxxExt`，称为 blanket implementations。
...
[cheats.rs](https://cheats.rs) 上有对应 blanket implementations 的说明（对应的说明是折叠的，入口 Working with Types-Types, Traits, Generics-Generics-Blanket Implementations）。

![image-20220108133742577](images/image-20220108133742577.png)

### traits with generic types

查看 `trait tower_service::Service<Request>` 的定义，使用了范型参数 `Request`，允许某个 service 的实现能处理多个不同的 `Request`；但对于某个确定的 `Request` 类型，只会返回对应的 `Respone` 类型，因此 `Response` 定义为关联类型，而非范型。
...
基于`trait tower_service::Service<Request>` 的定义，重新审视一下标准库中 `Iterator` trait 的定义：
...
使用关联类型，定义 Iterator 每次迭代返回的值；是否可以使用范型进行定义呢？
...
以 [Counter](https://doc.rust-lang.org/stable/book/ch13-02-iterators.html#creating-our-own-iterators-with-the-iterator-trait) 作为例子：
...
在使用范型定义时，为 `Counter` 实现 `Iterator<T>` trait，`impl Iterator<u32> for Counter` / `impl Iterator<String> for Counter`，可以为 `Counter` 实现多种类型的迭代器（multiple implementations of `Iterator` for `Counter`），在调用时，需要指定类型，[代码 12，use-generic-define-Iterator-trait](https://play.rust-lang.org/?version=stable&mode=debug&edition=2021&gist=3c0e439269e1e9076de77ab3f1df9bc7)。
...
在使用关联类型时，指定一个确定的返回值类型，因此只会有一种实现 `impl Iterator for Counter`，在调用时也就不用指定类型。
...
- 对于 `Iterator` trait，将集合作为迭代器进行访问，集合的每个元素类型固定，因此选择关联类型（associated types）更为合适；
- 对于 `trait tower_service::Service<Request>` trait，需要为不同的 `Request` 实现不同的 trait，因此需要范型参数（generic type parameter）。
...
定义 `IteratorExt` trait，提供 `our_flatten()` 方法的缺省实现；同时，为外部类型 `T`（使用 `where T: Iterator` 子句为 `T` 添加 trait bounds）实现 `IteratorExt` trait，这样就可以直接在 iterator 上调用我们实现的 `our_flatten()` 了，[代码 13，define-IteratorExt-trait](https://play.rust-lang.org/?version=stable&mode=debug&edition=2021&gist=99a1fd34e596705960b259d6a39e998c)。
...
`our_flatten()` 的最终实现中，我们添加了关于 `Sized` trait 的注释。
